...
class VideoService extends _$VideoService {
  ...
  @override
  FutureOr<List<VideoDatum>> build(int arg) {
    return getAllVideos();
  }

  Future<List<VideoDatum>> getAllVideos() async {
    var response = await dio.get(VIDEO_LIST, queryParameters: {'page': arg});
    return videosModelFromMap(jsonEncode(response.data)).data??[];
  }
